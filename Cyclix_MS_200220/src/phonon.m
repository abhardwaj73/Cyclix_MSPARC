...
% 			end
		...
		for kpt = 1:S.tnkpt
			...
			fac = 1.0i;
			[M1,M2] = div_gradvec_cychel(S,kpt_vec);
			PsiW = transpose(bsxfun(@times, conj(S.psi(:,1:S.Ns_nzocc(kpt),kpt)), S.W));
			DpsiW_x = transpose(bsxfun(@times, conj(blochGradient(S,kpt_vec,1)*S.psi(:,1:S.Ns_nzocc(kpt),kpt)),S.W));
			DpsiW_y = transpose(bsxfun(@times, conj(blochGradient(S,kpt_vec,2)*S.psi(:,1:S.Ns_nzocc(kpt),kpt)),S.W));
			DpsiW_z = transpose(bsxfun(@times, conj(blochGradient(S,kpt_vec,3)*S.psi(:,1:S.Ns_nzocc(kpt),kpt)),S.W));
			% Dpsi_xx = blochGradient(S,kpt_vec,1)*Dpsi_x;
			% Dpsi_xy = blochGradient(S,kpt_vec,1)*Dpsi_y;
			% Dpsi_xz = blochGradient(S,kpt_vec,1)*Dpsi_z;
			% Dpsi_yx = blochGradient(S,kpt_vec,2)*Dpsi_x;
			% Dpsi_yy = blochGradient(S,kpt_vec,2)*Dpsi_y;
			% Dpsi_yz = blochGradient(S,kpt_vec,2)*Dpsi_z;
			%Dpsi_zx = blochGradient(S,kpt_vec,3)*Dpsi_x;
			%Dpsi_zy = blochGradient(S,kpt_vec,3)*Dpsi_y;
			%Dpsi_zz = blochGradient(S,kpt_vec,3)*Dpsi_z;
			...
			for JJ_a = 1:S.n_atm % loop over all atoms
				...
				Chi_full_kR1 = zeros(S.N,S.Atom(JJ_a).angnum);
				Chi_full_kR2 = zeros(S.N,S.Atom(JJ_a).angnum);
				...
				for img = 1:S.Atom(JJ_a).n_image_rc
					img_disp = S.Atoms(JJ_a,:)-S.Atom(JJ_a).rcImage(img).coordinates;
		 			phase_fac_k = exp(-dot(kpt_vec,img_disp*fac));
		 			fac1 = round(img_disp(2)/S.L2);
					fac2 = round(img_disp(3)/S.L3);
					ROT = (S.RotM1^fac1) * (S.RotM2^fac2);
		 			Chi_full_kR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_kR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,1) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_k;
		 			Chi_full_kR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_kR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,2) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_k;
				...
				DChi_x = M1*Chi_full_kR1 + M2*Chi_full_kR2;
				DChi_y = -M1*Chi_full_kR2 + M2*Chi_full_kR1;
				DChi_z = blochGradient(S,kpt_vec,3)*Chi_full_k;
				...
				% % Chi_full_kR11 = zeros(S.N,S.Atom(JJ_a).angnum);
				% % Chi_full_kR12 = zeros(S.N,S.Atom(JJ_a).angnum);
				% % Chi_full_kR22 = zeros(S.N,S.Atom(JJ_a).angnum);
				% % for img = 1:S.Atom(JJ_a).n_image_rc
				% % 	img_disp = S.Atoms(JJ_a,:)-S.Atom(JJ_a).rcImage(img).coordinates;
				% % 	phase_fac_k = exp(-dot(kpt_vec,img_disp*fac));
		 	% % 		fac1 = round(img_disp(2)/S.L2);
				% % 	fac2 = round(img_disp(3)/S.L3);
				% % 	ROT = (S.RotM1^fac1) * (S.RotM2^fac2);
		 	% % 		Chi_full_kR11(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_kR11(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,1) * ROT(1,1) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_k;
		 	% % 		Chi_full_kR12(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_kR12(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,1) * ROT(1,2) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_k;
		 	% % 		Chi_full_kR22(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_kR22(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,2) * ROT(1,2) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_k;
		 	% % 	end
				% % DChi_x_kR1 = M1*Chi_full_kR11 + M2*Chi_full_kR12;
				% % DChi_x_kR2 = M1*Chi_full_kR12 + M2*Chi_full_kR22;
				% % DChi_xx = M1*DChi_x_kR1 + M2*DChi_x_kR2;
				% % DChi_y_kR1 = -M1*Chi_full_kR12 + M2*Chi_full_kR11;
				% % DChi_y_kR2 = -M1*Chi_full_kR22 + M2*Chi_full_kR12;
				% % DChi_yy = -M1*DChi_y_kR2 + M2*DChi_y_kR1;

				% % DChi_xy = M1*DChi_y_kR1 - M2*DChi_y_kR2;
				% % DChi_yx = DChi_xy;
				
				% % DChi_zx = blochGradient(S,kpt_vec,3) * DChi_x;
				% % DChi_xz = DChi_zx;
				% % DChi_zy = blochGradient(S,kpt_vec,3) * DChi_y;
				% % DChi_yz = DChi_zy;
				% % DChi_zz = blochGradient(S,kpt_vec,3) * DChi_z;
				...
				% 	ChiW = bsxfun(@times,S.Atom(JJ_a).rcImage(img).Chi_mat, S.W(S.Atom(JJ_a).rcImage(img).rc_pos)) * phase_fac_k;

				% 	integral_xx_temp = (Dpsi_xx(S.Atom(JJ_a).rcImage(img).rc_pos,:))' * ChiW;
				% 	integral_xy_temp = (Dpsi_xy(S.Atom(JJ_a).rcImage(img).rc_pos,:))' * ChiW;
				% 	integral_xz_temp = (Dpsi_xz(S.Atom(JJ_a).rcImage(img).rc_pos,:))' * ChiW;
				% 	integral_yx_temp = (Dpsi_yx(S.Atom(JJ_a).rcImage(img).rc_pos,:))' * ChiW;
				% 	integral_yy_temp = (Dpsi_yy(S.Atom(JJ_a).rcImage(img).rc_pos,:))' * ChiW;
				% 	integral_yz_temp = (Dpsi_yz(S.Atom(JJ_a).rcImage(img).rc_pos,:))' * ChiW;
				% 	integral_zz_temp = (Dpsi_zz(S.Atom(JJ_a).rcImage(img).rc_pos,:))' * ChiW;
						
				% 	integral_xx = integral_xx + ROT(1,1)*integral_xx_temp*ROT(1,1) + ROT(1,1)*integral_xy_temp*ROT(1,2) + ...
				% 					          + ROT(1,2)*integral_yx_temp*ROT(1,1) + ROT(1,2)*integral_yy_temp*ROT(1,2);
				% 	integral_xy = integral_xy + ROT(1,1)*integral_xx_temp*ROT(2,1) + ROT(1,1)*integral_xy_temp*ROT(2,2) + ...
				% 					          + ROT(1,2)*integral_yx_temp*ROT(2,1) + ROT(1,2)*integral_yy_temp*ROT(2,2);
				% 	integral_xz = integral_xz + ROT(1,1)*integral_xz_temp + ROT(1,2)*integral_yz_temp;
				% 	integral_yy = integral_yy + ROT(2,1)*integral_xx_temp*ROT(2,1) + ROT(2,1)*integral_xy_temp*ROT(2,2) + ...
				% 					          + ROT(2,2)*integral_yx_temp*ROT(2,1) + ROT(2,2)*integral_yy_temp*ROT(2,2);
				% 	integral_yz = integral_yz + ROT(2,1)*integral_xz_temp + ROT(2,2)*integral_yz_temp;
				% 	integral_zz = integral_zz + integral_zz_temp;				          				           
				% end

				% integral_yx = integral_xy;
				% integral_zx = integral_xz;
				% integral_zy = integral_yz;
		        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
                ...
                if S.BCx == 0
                    n_image_xl = floor((x0 + rcx)/S.L1);
                    n_image_xr = floor((S.L1 - x0+rcx-S.dx)/S.L1);
                else
                    n_image_xl = 0;
                    n_image_xr = 0;
                end
                ...
                if S.BCy == 0
                    n_image_yl = floor((y0 + rcy)/S.L2);
                    n_image_yr = floor((S.L2 - y0+rcy-S.dy)/S.L2);
                else
                    n_image_yl = 0;
                    n_image_yr = 0;
                end
                ...
                if S.BCz == 0
                    n_image_zl = floor((z0 + rcz)/S.L3);
                    n_image_zr = floor((S.L3 - z0+rcz-S.dz)/S.L3);
                else
                    n_image_zl = 0;
                    n_image_zr = 0;
                end
                ...
                for count_image = 1:n_image_total
                    ...
                    if(isImgRc)
                        ...
                        xx = S.xin + (ii_rc_s-S.FDn-1:ii_rc_e+S.FDn-1)*S.dx;
                        yy = S.yin + (jj_rc_s-S.FDn-1:jj_rc_e+S.FDn-1)*S.dy;
                        zz = S.zin + (kk_rc_s-S.FDn-1:kk_rc_e+S.FDn-1)*S.dz;

                        [XX_3D,YY_3D,ZZ_3D] = ndgrid(xx,yy,zz);
            
                        % Find distances
                        rc_dd_nl = calculateDistance(XX_3D,YY_3D,ZZ_3D,x0_i,y0_i,z0_i,S);
                        
                        XX_3D_vec = reshape(XX_3D,[],1);
                        YY_3D_vec = reshape(YY_3D,[],1);
                        ZZ_3D_vec = reshape(ZZ_3D,[],1);
                    
                        pos_node_cart = coordinateTransformation(S,[XX_3D_vec,YY_3D_vec,ZZ_3D_vec],'noncart2cart_dis');
                        ...
                        fac2 = (z0-z0_i)/S.L3;
                        ROT = (S.RotM1^fac1) * (S.RotM2^fac2);
                        P1 = ROT(1,1)*rc_XX + ROT(1,2)*rc_YY;
                        P2 = ROT(2,1)*rc_XX + ROT(2,2)*rc_YY;
                        rc_XX = P1; rc_YY = P2;

                        rc_XX = reshape(rc_XX,size(rc_dd_nl,1),size(rc_dd_nl,2),size(rc_dd_nl,3));
                        rc_YY = reshape(rc_YY,size(rc_dd_nl,1),size(rc_dd_nl,2),size(rc_dd_nl,3));
                        rc_ZZ = reshape(rc_ZZ,size(rc_dd_nl,1),size(rc_dd_nl,2),size(rc_dd_nl,3));


                        [II_rc,JJ_rc,KK_rc] = ndgrid(ii_rc_s:ii_rc_e,jj_rc_s:jj_rc_e,kk_rc_s:kk_rc_e);
                        Rowcount_rc = (KK_rc-1)*S.Nx*S.Ny + (JJ_rc-1)*S.Nx + II_rc;

                        II = 1+S.FDn : size(rc_dd_nl,1)-S.FDn;
                        JJ = 1+S.FDn : size(rc_dd_nl,2)-S.FDn;
                        KK = 1+S.FDn : size(rc_dd_nl,3)-S.FDn;

                        img_disp = [x0 y0 z0] - [x0_i y0_i z0_i];
                        phase_fac_k = exp(-dot(kpt_vec,img_disp*fac));

                        if S.Atm(count_typ).psptyp == 0
                            angnum = (lmax^2 + 2*lmax + 1) - (2*lloc + 1);
                            count_pp = 1;
                            for l=0:lmax
                                if l~=lloc
                                    ...
                                    for m=-l:l
                                        ...
                                        Chi_mat = UdV_Jl .* Ylm;
                                        [dChiJ_x, dChiJ_y, dChiJ_z] = dpseudopot(Chi_mat,II,JJ,KK,XX_3D(II,JJ,KK),YY_3D(II,JJ,KK),ZZ_3D(II,JJ,KK),S);
                                        
                                        for ns = 1:S.Ns_nzocc(kpt)
                                            dpsiW_x = DpsiW_x(ns,:);dpsiW_y = DpsiW_y(ns,:);dpsiW_z = DpsiW_z(ns,:);
                                            integral_xx(ns,count_pp) = integral_xx(ns,count_pp) - (ROT(1,1) * sum(sum(sum(dpsiW_x(Rowcount_rc).*dChiJ_x(II,JJ,KK)))) * ROT(1,1) + ROT(1,1) * sum(sum(sum(dpsiW_x(Rowcount_rc).*dChiJ_y(II,JJ,KK)))) * ROT(1,2) + ...
                                                                      + ROT(1,2) * sum(sum(sum(dpsiW_y(Rowcount_rc).*dChiJ_x(II,JJ,KK)))) * ROT(1,1) + ROT(1,2) * sum(sum(sum(dpsiW_y(Rowcount_rc).*dChiJ_y(II,JJ,KK)))) * ROT(1,2)) * phase_fac_k;
                                            integral_xy(ns,count_pp) = integral_xy(ns,count_pp) - (ROT(1,1) * sum(sum(sum(dpsiW_x(Rowcount_rc).*dChiJ_x(II,JJ,KK)))) * ROT(2,1) + ROT(1,1) * sum(sum(sum(dpsiW_x(Rowcount_rc).*dChiJ_y(II,JJ,KK)))) * ROT(2,2) + ...
                                                                      + ROT(1,2) * sum(sum(sum(dpsiW_y(Rowcount_rc).*dChiJ_x(II,JJ,KK)))) * ROT(2,1) + ROT(1,2) * sum(sum(sum(dpsiW_y(Rowcount_rc).*dChiJ_y(II,JJ,KK)))) * ROT(2,2)) * phase_fac_k;
                                            integral_xz(ns,count_pp) = integral_xz(ns,count_pp) - (ROT(1,1) * sum(sum(sum(dpsiW_x(Rowcount_rc).*dChiJ_z(II,JJ,KK)))) + ROT(1,2) * sum(sum(sum(dpsiW_y(Rowcount_rc).*dChiJ_z(II,JJ,KK))))) * phase_fac_k;
                                            integral_yy(ns,count_pp) = integral_yy(ns,count_pp) - (ROT(2,1) * sum(sum(sum(dpsiW_x(Rowcount_rc).*dChiJ_x(II,JJ,KK)))) * ROT(2,1) + ROT(2,1) * sum(sum(sum(dpsiW_x(Rowcount_rc).*dChiJ_y(II,JJ,KK)))) * ROT(2,2) + ...
                                                                      + ROT(2,2) * sum(sum(sum(dpsiW_y(Rowcount_rc).*dChiJ_x(II,JJ,KK)))) * ROT(2,1) + ROT(2,2) * sum(sum(sum(dpsiW_y(Rowcount_rc).*dChiJ_y(II,JJ,KK)))) * ROT(2,2)) * phase_fac_k;
                                            integral_yz(ns,count_pp) = integral_yz(ns,count_pp) - (ROT(2,1) * sum(sum(sum(dpsiW_x(Rowcount_rc).*dChiJ_z(II,JJ,KK)))) + ROT(2,2) * sum(sum(sum(dpsiW_y(Rowcount_rc).*dChiJ_z(II,JJ,KK))))) * phase_fac_k;
                                            integral_zz(ns,count_pp)= integral_zz(ns,count_pp) - sum(sum(sum(dpsiW_z(Rowcount_rc).*dChiJ_z(II,JJ,KK))))* phase_fac_k;
                                        end
                                        count_pp = count_pp + 1;                                           
                                    end    
                                end
				...
				S.M_dyn_nl(3*(JJ_a-1)+1,3*(JJ_a-1)+1) = S.M_dyn_nl(3*(JJ_a-1)+1,3*(JJ_a-1)+1) + 2 * S.occfac * wkpt(kpt) * ...
		        ...
		        S.M_dyn_nl(3*(JJ_a-1)+1,3*(JJ_a-1)+2) = S.M_dyn_nl(3*(JJ_a-1)+1,3*(JJ_a-1)+2) + 2 * S.occfac * wkpt(kpt) * ...
				...
				S.M_dyn_nl(3*(JJ_a-1)+1,3*(JJ_a-1)+3) = S.M_dyn_nl(3*(JJ_a-1)+1,3*(JJ_a-1)+3) + 2 * S.occfac * wkpt(kpt) * ...
				...
				S.M_dyn_nl(3*(JJ_a-1)+2,3*(JJ_a-1)+1) = S.M_dyn_nl(3*(JJ_a-1)+2,3*(JJ_a-1)+1) + 2 * S.occfac * wkpt(kpt) * ...
		        ...
		        S.M_dyn_nl(3*(JJ_a-1)+2,3*(JJ_a-1)+2) = S.M_dyn_nl(3*(JJ_a-1)+2,3*(JJ_a-1)+2) + 2 * S.occfac * wkpt(kpt) * ...
				...
				S.M_dyn_nl(3*(JJ_a-1)+2,3*(JJ_a-1)+3) = S.M_dyn_nl(3*(JJ_a-1)+2,3*(JJ_a-1)+3) + 2 * S.occfac * wkpt(kpt) * ...
				...
				S.M_dyn_nl(3*(JJ_a-1)+3,3*(JJ_a-1)+1) = S.M_dyn_nl(3*(JJ_a-1)+3,3*(JJ_a-1)+1) + 2 * S.occfac * wkpt(kpt) * ...
		        ...
		        S.M_dyn_nl(3*(JJ_a-1)+3,3*(JJ_a-1)+2) = S.M_dyn_nl(3*(JJ_a-1)+3,3*(JJ_a-1)+2) + 2 * S.occfac * wkpt(kpt) * ...
				...
				S.M_dyn_nl(3*(JJ_a-1)+3,3*(JJ_a-1)+3) = S.M_dyn_nl(3*(JJ_a-1)+3,3*(JJ_a-1)+3) + 2 * S.occfac * wkpt(kpt) * ...
...
function [S] = Calculate_dynamical_matrix(S,scf_natmc)
	...
	for atmc = 1:scf_natmc
		...
		for kpt = 1:S.tnkpt
	        ...
	        elseif Dir == 1
	        	Chi_full_kR1 = zeros(S.N,S.Atom(JJ_a).angnum);
				Chi_full_kR2 = zeros(S.N,S.Atom(JJ_a).angnum);
				Chi_full_kqR1 = zeros(S.N,S.Atom(JJ_a).angnum);
				Chi_full_kqR2 = zeros(S.N,S.Atom(JJ_a).angnum);
				[M1_k,M2_k] = div_gradvec_cychel(S,S.kptgrid(kpt,:));
				[M1_kq,M2_kq] = div_gradvec_cychel(S,S.kqptgrid(kpt,:));
				...
				for img = 1:S.Atom(JJ_a).n_image_rc
					...
					ROT = (S.RotM1^fac1) * (S.RotM2^fac2);
		 			Chi_full_kR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_kR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,1) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_k;
		 			Chi_full_kR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_kR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,2) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_k;
		 			Chi_full_kqR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_kqR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,1) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_kq;
		 			Chi_full_kqR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_kqR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,2) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_kq;
		 		end
				DChi_k = M1_k*Chi_full_kR1 + M2_k*Chi_full_kR2;
				DChi_kq = M1_kq*Chi_full_kqR1 + M2_kq*Chi_full_kqR2;
			else
				Chi_full_kR1 = zeros(S.N,S.Atom(JJ_a).angnum);
				Chi_full_kR2 = zeros(S.N,S.Atom(JJ_a).angnum);
				Chi_full_kqR1 = zeros(S.N,S.Atom(JJ_a).angnum);
				...
				DChi_k = -M1_k*Chi_full_kR2 + M2_k*Chi_full_kR1;
				DChi_kq = -M1_kq*Chi_full_kqR2 + M2_kq*Chi_full_kqR1;
			end
    ...
    if S.Kdeltaq0 == 0
        for atmc = 1:scf_natmc
            ...
            for kpt = 1:S.tnkpt    
		        ...
		        elseif Dir == 1
		        	Chi_full_mkR1 = zeros(S.N,S.Atom(JJ_a).angnum);
					Chi_full_mkR2 = zeros(S.N,S.Atom(JJ_a).angnum);
					Chi_full_mkqR1 = zeros(S.N,S.Atom(JJ_a).angnum);
					Chi_full_mkqR2 = zeros(S.N,S.Atom(JJ_a).angnum);
					[M1_mk,M2_mk] = div_gradvec_cychel(S,-S.kptgrid(kpt,:));
					[M1_mkq,M2_mkq] = div_gradvec_cychel(S,S.mkqptgrid(kpt,:));
					...
					for img = 1:S.Atom(JJ_a).n_image_rc
						...
						fac2 = round(img_disp(3)/S.L3);
						ROT = (S.RotM1^fac1) * (S.RotM2^fac2);
			 			Chi_full_mkR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_mkR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,1) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_mk;
			 			Chi_full_mkR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_mkR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,2) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_mk;
			 			Chi_full_mkqR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_mkqR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,1) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_mkq;
			 			Chi_full_mkqR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_mkqR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,2) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac_mkq;
			 		end
					DChi_mk = M1_mk*Chi_full_mkR1 + M2_mk*Chi_full_mkR2;
					DChi_mkq = M1_mkq*Chi_full_mkqR1 + M2_mkq*Chi_full_mkqR2;
				else
					Chi_full_mkR1 = zeros(S.N,S.Atom(JJ_a).angnum);
					Chi_full_mkR2 = zeros(S.N,S.Atom(JJ_a).angnum);
					Chi_full_mkqR1 = zeros(S.N,S.Atom(JJ_a).angnum);
					...
					DChi_mk = -M1_mk*Chi_full_mkR2 + M2_mk*Chi_full_mkR1;
					DChi_mkq = -M1_mkq*Chi_full_mkqR2 + M2_mkq*Chi_full_mkqR1;
				end
	...
	for kpt = 1:S.tnkpt
		...
		kqpt_vec = S.kqptgrid(kpt,:);
		fac = 1.0i;
		[M1_k,M2_k] = div_gradvec_cychel(S,kpt_vec);
		[M1_kq,M2_kq] = div_gradvec_cychel(S,kqpt_vec);
		...
		for JJ_a = 1:S.n_atm % loop over all atoms
			...
			DChi_x = M1_k*Chi_full_kR1 + M2_k*Chi_full_kR2;
			DChi_kqx = M1_kq*Chi_full_kqR1 + M2_kq*Chi_full_kqR2;
			DChi_y = -M1_k*Chi_full_kR2 + M2_k*Chi_full_kR1;
			DChi_kqy = -M1_kq*Chi_full_kqR2 + M2_kq*Chi_full_kqR1;
			DChi_z = blochGradient(S,kpt_vec,3) * Chi_full_k;
			DChi_kqz = blochGradient(S,kqpt_vec,3) * Chi_full_kq;
    ...
    if S.Kdeltaq0 == 0
        for kpt = 1:S.tnkpt
            ...
            mkqpt_vec = S.mkqptgrid(kpt,:);
            fac = 1.0i;
            [M1_mk,M2_mk] = div_gradvec_cychel(S,mkpt_vec);
		    [M1_mkq,M2_mkq] = div_gradvec_cychel(S,mkqpt_vec);
            ...
            for JJ_a = 1:S.n_atm % loop over all atoms
				...
				DChi_x = M1_mk*Chi_full_mkR1 + M2_mk*Chi_full_mkR2;
				DChi_kqx = M1_mkq*Chi_full_mkqR1 + M2_mkq*Chi_full_mkqR2;
				DChi_y = -M1_mk*Chi_full_mkR2 + M2_mk*Chi_full_mkR1;
				DChi_kqy = -M1_mkq*Chi_full_mkqR2 + M2_mkq*Chi_full_mkqR1;
				DChi_z = blochGradient(S,mkpt_vec,3) * Chi_full_mk;
				DChi_kqz = blochGradient(S,mkqpt_vec,3) * Chi_full_mkq;
...
function S = sternheimer(S,scf_natmc,qpt)
	...
	if S.Kdeltaq0 == 1
		for kpt = 1:S.tnkpt
	        ...
	        if Dir == 3
	        	DChi = blochGradient(S,kpt_vec,3) * Chi_full;
	        elseif Dir == 1
	        	Chi_full_kR1 = zeros(S.N,S.Atom(JJ_a).angnum);
				Chi_full_kR2 = zeros(S.N,S.Atom(JJ_a).angnum);
				[M1,M2] = div_gradvec_cychel(S,kpt_vec);
				for img = 1:S.Atom(JJ_a).n_image_rc
					...
					fac2 = round(img_disp(3)/S.L3);
					ROT = (S.RotM1^fac1) * (S.RotM2^fac2);
		 			Chi_full_kR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_kR1(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,1) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac;
		 			Chi_full_kR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) = Chi_full_kR2(S.Atom(JJ_a).rcImage(img).rc_pos,:) + ROT(1,2) * S.Atom(JJ_a).rcImage(img).Chi_mat * phase_fac;
		 		end
				DChi = M1*Chi_full_kR1 + M2*Chi_full_kR2;
			else
				Chi_full_kR1 = zeros(S.N,S.Atom(JJ_a).angnum);
				Chi_full_kR2 = zeros(S.N,S.Atom(JJ_a).angnum);
				[M1,M2] = div_gradvec_cychel(S,kpt_vec);
				...
				DChi = -M1*Chi_full_kR2 + M2*Chi_full_kR1;
			end
...
function S = electrostatics_local(S,qpt)
	...
	M_dyn_temp1 = zeros(3*S.n_atm,S.N);
	M_dyn_temp2 = zeros(S.N,3*S.n_atm);
	...
	for JJ_a = 1:S.n_atm % loop over all the atoms
		...
		for count_image = 1:n_image_total
			...
			fac1 = round(-(y0-y0_i)/S.L2);
			fac2 = round(-(z0-z0_i)/S.L3);
			ROT = (S.RotM1^fac1) * (S.RotM2^fac2);
			M_dyn_elec_temp = (M_dyn_elec_temp + M_dyn_elec_temp')/2;
