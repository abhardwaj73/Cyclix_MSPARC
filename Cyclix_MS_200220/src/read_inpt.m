...
while(~feof(fid1))
	...
	elseif (strcmp(str,'OUTPUT_FILE:'))	  
		...
		textscan(fid1,'%s',1,'delimiter','\n','MultipleDelimsAsOne',0); % skip current line
	elseif (strcmp(str,'QPOINT_GRID:'))	
		C_param = textscan(fid1,'%f %f %f',1,'delimiter',' ','MultipleDelimsAsOne',1);	
		S.qptgrid = cell2mat(C_param);	
		textscan(fid1,'%s',1,'delimiter','\n','MultipleDelimsAsOne',0); % skip current line 	
	else 
...
if (S.BC >= 0 && S.BC <= 4) && (S.BCx >= 0 && S.BCy >= 0 && S.BCz >= 0)
...
% Pressure calculation only for crystal and stress calculation non existent
% for isolated clusters

dim = 3;
if (S.BCx >= 0)
    dim = 3 - (S.BCx+S.BCy+S.BCz);
end

 if((dim == 0 || S.BC == 1) && S.Calc_stress == 1)
    error('\nStress does not exist in an isolated cluster!\n');
end


 if((dim ~= 3 || (S.BC > 0 && S.BC ~= 2) ) && S.Calc_pres == 1)
    error('\nPressure exist only in all periodic systems!\n');
end

 if (Flag_tolrelaxcell == 0 && (dim ~= 3 || (S.BC > 0 && S.BC ~= 2)))
    if (dim == 1 || (S.BC >= 4 && S.BC <= 7))
        S.TOL_RELAX_CELL = 5e-5;
    elseif (dim == 2 || S.BC == 3)
        S.TOL_RELAX_CELL = 5e-6;
    end
end

 % Restart option unavaiable for RelaxFlag = 2,3
if (S.RelaxFlag == 2 || S.RelaxFlag == 3) && S.RestartFlag == 1
    error('Restart option unavaiable for cell-relaxation and full relaxation currently\n');
end
