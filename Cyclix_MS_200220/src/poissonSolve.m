...
if S.cell_typ < 3
	...
	% find radial boundary condition
		% Calculate phi
		phi = zeros(S.Nx+2*S.FDn, S.Ny, S.Nz);

		%reshape rho to 3D 
		rho = reshape(f, S.Nx, S.Ny*S.Nz); % note here after rho = rho + b
		
		%sum over Z direction, \int (\rho) dz / LZ
		r = [S.xin + (0:S.Nx-1) * S.dx]'; 
		
		Intg = (1/2/pi/S.L2/S.L3)*sum(sum(rho,2).*log(r)) * S.dx *S.dy * S.dz;
		...
		R_inv = 1./repmat(r,[1,S.Ny,S.Nz]);

		for p = 1:S.FDn
			d = d - S.w2(p+1)/dx2 * (phi(II+p,JJ,KK) + phi(II-p,JJ,KK)) - S.w1(p+1)/S.dx * (phi(II+p,JJ,KK) - phi(II-p,JJ,KK)).* R_inv;
		end
		fprintf('\nPoisson RHS done\n');
end
