...
function [S] = LDA_PZ(S)
	...
	func_id_x = 1; % Purdew-Zunger Exchange functional
    ...
    [e_x, v_x, dvxdrho, v3rho3] = xc(func_id_x, rho);% e_x = exchange energy per unit particle, v_x = d(rho*e_x)/drho = e_x + rho*de_x/drho, dvxdgrho = d(rho*e_x)/dsigma = rho*de_x/dsigma + 0

    func_id_c = 9; % Purdew-Zunger Correlation functional
    mex -I/home/users/asharma424/Downloads/Softwares/libxc-4.3.3/src -I/home/users/asharma424/Downloads/Softwares/libxc-4.3.3 -L/usr/lib64 -lxc -lm /home/users/asharma424/Downloads/Softwares/libxc-4.3.3/examples/xc.c ;
    [e_c, v_c, dvcdrho, v3rho3] = xc(func_id_c, rho);% e_c = correlation energy per unit particle, v_c = d(rho*e_c)/drho = e_c + rho*de_c/drho, dvcdgrho = d(rho*e_c)/dsigma = rho*de_c/dsigma + 0
    fprintf('\n\n');
    S.e_xc = e_x + e_c ;
    S.Vxc = v_x + v_c ;
    S.Dvxc_drho = dvxdrho + dvcdrho;
...
%                      S.lapc_T(3,2)*grad_3*(S.dvxcdgrho.*drho_2) + S.lapc_T(3,1)*grad_1*(S.dvxcdgrho.*drho_3) + S.lapc_T(3,1)*grad_3*(S.dvxcdgrho.*drho_1) );
	...
	elseif (S.cell_typ >= 3 && S.cell_typ <= 5)
		%S.Vxc = v_xc - S.div_gradvec_1 * (S.dvxcdgrho.*drho_1) - S.div_gradvec_2 * (S.dvxcdgrho.*drho_2) - S.grad_3 * (S.dvxcdgrho.*drho_3); %ARPIT
		S.Vxc = v_xc - S.grad_1 * (S.dvxcdgrho.*drho_1) - S.grad_2 * (S.dvxcdgrho.*drho_2) - S.grad_3 * (S.dvxcdgrho.*drho_3);
	end
end
%--------------------------------------------------------------------------
...
function [S] = GSGA_PBE(S,XC)
	...
	elseif S.cell_typ == 2
		Vxc_temp = S.lapc_T(1,1)*S.grad_1*(S.dvxcdgrho.*drho_1) + S.lapc_T(2,2)*S.grad_2*(S.dvxcdgrho.*drho_2) + S.lapc_T(3,3)*S.grad_3*(S.dvxcdgrho.*drho_3) +...
				   ...
				   S.lapc_T(3,2)*S.grad_3*(S.dvxcdgrho.*drho_2) + S.lapc_T(3,1)*S.grad_1*(S.dvxcdgrho.*drho_3) + S.lapc_T(3,1)*S.grad_3*(S.dvxcdgrho.*drho_1) ;
	elseif (S.cell_typ >= 3 && S.cell_typ <= 5)
		Vxc_temp = S.div_gradvec_1 * (S.dvxcdgrho.*drho_1) + S.div_gradvec_2 * (S.dvxcdgrho.*drho_2) + S.grad_3 * (S.dvxcdgrho.*drho_3); 
