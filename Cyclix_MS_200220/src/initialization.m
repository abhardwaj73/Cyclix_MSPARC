...
elseif strcmp(S.XC, 'GGA_PBE') || strcmp(S.XC, 'GGA_RPBE') || strcmp(S.XC, 'GGA_PBEsol')
 ...
 error('Unrecognized xc functional\n');
end
...
if S.bet < 0
	if S.elec_T_type == 0 % fermi-dirac 
		...
		S.bet = S.Cst / 0.1; % smearing = 0.1 eV = 0.00367493225 Ha, Beta := 1 / smearing
	elseif S.elec_T_type == 1 % gaussian smearing
		% The electronic temperature corresponding to 0.2 eV is 2320.904422 K
		S.bet = S.Cst / 0.2; % smearing = 0.2 eV = 0.00734986450 Ha, Beta := 1 / smearing
	end
	S.Temp = 1./(S.kB * S.bet); 
end
...
elseif (S.RelaxFlag == 2 || S.RelaxFlag == 3)
    S.Printrestart = 0;
end
...
if S.xc == 2 && (S.cell_typ >= 3 && S.cell_typ <= 5)
...
S = struct(...
	...
	'pseudocharge_tol',pseudocharge_tol, 'Cst',Cst,'Ha_Bohr3_GPa',Ha_Bohr3_GPa,'kB',kB,'elec_T_type',elec_T_type,...
...
S.TOL_RELAX_CELL = 0.01; % in GPa (for all periodic)
S.MDFlag = 0;
S.RestartFlag = 0;
S.MDMeth = 'NVE';
S.MD_dt = 1.0;
...
fprintf(fileID,'***************************************************************************\n');
fprintf(fileID,'*                          M-SPARC (Feb 20, 2020)                         *\n');
...
if (S.cell_typ <= 2)
	...
	fprintf(fileID,'\n');
elseif (S.cell_typ >= 3 && S.cell_typ <= 5)
	str_BC = ['C', 'D'];
	fprintf(fileID,'BC:');
	fprintf(fileID,' %s',str_BC(S.BCx+1));
	fprintf(fileID,' %s',str_BC(S.BCy+1));
...
if (S.RelaxFlag == 1 || S.RelaxFlag == 3)
...
S.DDBfname = strcat(filename,'.DDB');
...
function [S] = Generate_kpts(S)
	...
	MPG_typ1 = @(nkpt) (-floor((nkpt - 1)/2):-floor((nkpt - 1)/2)+nkpt-1); % MP grid points for infinite group order (same as that in ABINIT)
	...
	kptgrid_y = round((2*pi/S.L2) * kptgrid_y);
	...
	if S.TimeRevSym == 1
		...
		for ii = 1:tnkpt
			for jj = ii+1:tnkpt
				if (abs(kptgrid(ii,1) + kptgrid(jj,1) - sumx) < TOL) && (abs(kptgrid(ii,2) + kptgrid(jj,2) - sumy) < TOL || abs(kptgrid(ii,2) + kptgrid(jj,2)) < TOL ) && (abs(kptgrid(ii,3) + kptgrid(jj,3) - sumz) < TOL)
					Ikpt(ii) = 1;
					Ikpt_rev(jj) = 1;
				end
			end
