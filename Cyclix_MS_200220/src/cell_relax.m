...
lower_bound =  V / (S.max_dilatation)^S.cellrelax_ndim;
upper_bound =  V * (S.max_dilatation)^S.cellrelax_ndim;
...
if S.RelaxFlag == 2
    ...
    [~,~,S] = electronicGroundState(S.Atoms,S);
    % print the result in .geopt file
    if S.PrintRelaxout == 1
        PrintCellRelax(S);
    end
    S.elecgs_Count = S.elecgs_Count + 1;
    S.Relax_iter = S.Relax_iter + 1;
elseif S.RelaxFlag == 3 % Perform atomic relaxation before for this cell for full relaxation
    S.RelaxFlag = 1;
    S.Calc_stress = 1;
    S.StressCount = S.elecgs_Count;
    S = relax(S);
    S.RelaxFlag = 3;
end
...
stress_lc = S.Stress;
if(S.cell_typ == 2)
    latuvec_red = S.stress_dimred * S.lat_uvec * S.stress_dimred';
    stress_lc = latuvec_red * S.Stress * inv(latuvec_red);
end
...
%else
%    max_P_stress = S.Stress;
%end
max_P_stress = trace(stress_lc)/S.cellrelax_ndim;
