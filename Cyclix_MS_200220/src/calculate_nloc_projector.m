	...
	for count_image = 1:n_image_total
		...
		if(isImgRc)
			...
			if(rc_n_nodes~=0)
				...
				% Atom(JJ_a).rcImage(imgRcCount).rc_ii_s = ii_rc_s;
				% Atom(JJ_a).rcImage(imgRcCount).rc_ii_e = ii_rc_e;
				% Atom(JJ_a).rcImage(imgRcCount).rc_jj_s = jj_rc_s;
				% Atom(JJ_a).rcImage(imgRcCount).rc_jj_e = jj_rc_e;
				% Atom(JJ_a).rcImage(imgRcCount).rc_kk_s = kk_rc_s;
				% Atom(JJ_a).rcImage(imgRcCount).rc_kk_e = kk_rc_e;
